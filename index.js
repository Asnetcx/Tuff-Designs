...
// =====================
const CHANNELS = {
  PAYMENTS: "1481801328257925141",
  COMMISSION: "1481801397438517269",
  TICKET: "1481801500194902129",
  QUEUE: "1481801642058715249",
  ANNOUNCE: "1476353438844715068"
};
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (["logo_update","banner_update","clothing_update"].includes(cmd)) {
        ...
        // Log to Commission channel
        const commissionChannel = interaction.guild.channels.cache.get(CHANNELS.COMMISSION);
        if (commissionChannel) commissionChannel.send({ embeds: [embed] });
      ...
      if (cmd === "payout") {
        ...
        const paymentsChannel = interaction.guild.channels.cache.get(CHANNELS.PAYMENTS);
        if (paymentsChannel) paymentsChannel.send({ embeds: [embed] });
      ...
      if (cmd === "timeout") {
        ...
        await member.timeout(10 * 60 * 1000, `Timeout by ${interaction.user.tag}`);
